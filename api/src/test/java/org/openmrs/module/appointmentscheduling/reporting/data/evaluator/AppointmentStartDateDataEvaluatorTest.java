...
public class AppointmentStartDateDataEvaluatorTest extends BaseModuleContextSensitiveTest {
    ...
    @Test
    public void shouldFetchStartDateFromAppointment() throws Exception {

        AppointmentEvaluationContext context = new AppointmentEvaluationContext();
        context.setBaseAppointments(new AppointmentIdSet(8));

        EvaluatedAppointmentData result = appointmentDataService.evaluate(new AppointmentStartDateDataDefinition(), context);

        assertThat(result.getData().size(), is(1));
        assertThat(((Date) result.getData().get(8)).getTime(), is(new DateTime(2014,1,2,0,0,0).getMillis()));
