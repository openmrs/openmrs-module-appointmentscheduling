    ...
    public PageableResult search(RequestContext context) throws ResponseException {
        ...
        if (StringUtils.isNotBlank(statusValue)) {
            if (statusValue.equals("LATE")){
                return new NeedsPaging<AppointmentData>(Context.getService(AppointmentService.class).getLateAppointments(
                        fromDate, toDate, location, provider, appointmentType), context);
            }else if (statusValue.equals("EARLY")){
                return new NeedsPaging<AppointmentData>(Context.getService(AppointmentService.class).getEarlyAppointments(
                        fromDate, toDate, location, provider, appointmentType), context);
            }else {
                return null;
            }
